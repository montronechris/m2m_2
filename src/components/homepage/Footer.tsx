export default function Footer() {
  const cols = [
    { h: "Navigazione", links: ["Home", "Funzioni", "Sicurezza", "Demo", "Contatti"] },
    { h: "Legale", links: ["Privacy Policy", "Termini di Servizio", "Cookie Policy", "GDPR"] },
  ];
  return (
    <footer className="relative z-10 border-t border-ink/10 px-6 py-16 bg-cream">
      <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-4">
        <div className="md:col-span-2">
          ...
          <p className="mt-4 max-w-sm text-sm text-ink/50">
            La soluzione completa per la gestione degli ordini nella ristorazione moderna.
            Semplifichiamo la comanda, ottimizziamo il servizio e proteggiamo i tuoi dati.
          </p>
        </div>
        {cols.map((c) => (
          <div key={c.h}>
      ...
      <div className="mx-auto mt-12 flex max-w-6xl flex-col items-center justify-between gap-3 border-t border-ink/10 pt-8 text-xs text-ink/40 sm:flex-row">
        <span>© 2026 TavolaRapida S.r.l. — Tutti i diritti riservati.</span>
